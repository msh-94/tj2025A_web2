...
// ************* ws 프로토콜 통신이 왔을대 특정한 핸들러(클래스) 로 매핑/연결 ************* //
...
public class WebSocketConfig implements WebSocketConfigurer {// class start
    ...
    @Autowired private ChatHandler chatHandler; // DI 서버웹소켓  객체
    // 1. 개발자가 만든 서버웹소켓(핸들러) 객체를 스프링이 알수 있게 경로 등록한다
    @Override
    public void registerWebSocketHandlers(WebSocketHandlerRegistry registry) {
        // 내가(개발자) 가 만든 서버웹소켓(핸들러) 주소 와 함께 등록한다.
