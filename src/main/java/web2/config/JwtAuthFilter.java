...
public class JwtAuthFilter extends OncePerRequestFilter { // class start
    // 내가만든 토큰(jwtService) 인증 방법을 (시큐리티방식)UsernamePasswordAuthenticationToken 혼합
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        // 2. UsernamePasswordAuthenticationToken 을 재 정의하기
        if (token != null && jwtService.checkToken(token)){ // 토큰이 유효 하면
            ...
            // new UsernamePasswordAuthenticationToken( "id" , "password" , "권한");
            // 2-2 : 시큐리티 토큰 생성
