...
public class SecurityConfig { // class start
    // 내가만든 토큰을 시큐리티 토큰 방식으로 통합한 클래스
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception{
        ...
        http.authorizeHttpRequests( auth -> auth
                ...
                .requestMatchers("/api/admin/**").hasRole("ADMIN") // admin 관련 controller는 "ADMIN"권한일때 허용
                .requestMatchers("/**").permitAll() ); // 모든 권한 허용은 항상 최하단에 정의한다

                // [2] HTTP 요청에 csrf( 요청간의 해킹 공격 ) POST, PUT 자동차단 커스텀
        ...
        // [5] 시큐리티의 CORS 정책을 기본(개발자가 설정한 CorsConfig 클래스) 설정하기
