...
public interface TodoRepository extends JpaRepository<TodoEntity , Integer> {
    ...
    // 2-1 : findByTitle( String 매개변수 ) <---> select * from todo where title = 매개변수
    List<TodoEntity> findByTitle( String title ); // findByXXX( String 매개변수 ) ,
    ...
    // 3. 네이티브 쿼리 메소드
    // * SQL 문장을 직접 작성하여 실행한다 , :(콜론)매개변수 이용하여 매개변수 대입한다
    // 3-1 : @Query( value = "SQL작성" , nativeQuery = true )
