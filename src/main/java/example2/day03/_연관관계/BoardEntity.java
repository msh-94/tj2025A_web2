...
public class BoardEntity {
    ...
    // * 단방향 연결 * //
    // 하위 엔티티가 상위 엔티티 참조관계
    @ManyToOne( cascade = CascadeType.ALL , fetch = FetchType.LAZY ) // M:1 , 다수 FK가 1개의 PK 에게]
...
/*
    [ 영속성 ] 자바 데이터(객체) 와 데이터베이스 데이터(테이블/레코드) 를 매핑(연결) 하여
    ...
    [ fetch ] 부모[PK] 와 자식[FK] 관계에서 엔티티를 조회하는 방법
    ...
    * 한번 SELECT(조회) 된 결과는 MYBATIS 와 JPA 는 자동 1차 캐싱(기록)
