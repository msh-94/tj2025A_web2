...
export default function Login( props ){
    ...
    const postLogin = async () => {
        try{
            ...
            if( res.data != '' ){
                alert('로그인 성공');
                location.href='/'; // location.href 이용한 페이지 전체 랜더링
            }else{ alert('로그인 실패'); }
        }catch(e){}
    }// func end
    ...
    return(
        ...
        <h3> 로그인 페이지 </h3>
        <form>
            아이디 : <input value={ uid } onChange={ (e) => setUid( e.target.value ) }/> <br/>
            비밀번호 : <input type="password" value={ upwd } onChange={ (e) => setUpwd( e.target.value ) }/> <br/>
