...
export default function Component12( props ){
    ...
    function Product( props ){
        ...
        const { name , price } = useParams(); // 4-1 const { 매개변수명1 , 매개변수명2 } = useParams();
        return (
            <>
            <h3> 제품 소개 페이지 </h3>
    ...
    function Page404( props ){
        ...
        const 이동함수 = () => {
            ...
            // 5-2 useNavigate() 반환값 인 navigate 사용한다
            navigate("/");
        }
        return (
            <>
            <h3> 존재 하지 않는 페이지 입니다. </h3>
            <a href='/'>홈으로1(get방식)</a>
    ...
    return ( 
        ...
        <BrowserRouter>
            <ul>
                ...
                <Link to="/product"> 제품 소개 페이지1 (path x) </Link>
                <Link to="/product/코카콜라/1000"> 제품 소개 페이지2(path o) </Link>
            </ul>
            <Routes> { /* 가상의 URL 정의하고 정의한 URL 과 매핑할 컴포넌트 정의 */ }
                ...
                {/* 만약에 존재하지 않는 가상 URL 요청하면 404 컴포넌트 */}
