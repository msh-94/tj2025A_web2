...
export default function Component8( props ){
    ...
    // [1] useEffect : 특정한( 현재 컴포넌트=실행/종료 ) 시점에 정의함수가 실행
    // [2] 사용법 :
    //      2-0 : import { useEffect } from "react"
    //      2-1 mount/update  : useEffect( 정의함수명 ) 또는 useEffect( () => { } )
    //      2-2 mount/update* : useEffect( () => { } , [상태변수명] )           
    ...
    useEffect( () => { console.log('(3-2)컴포넌트(함수) 탄생 , *특정* 업데이트'); } , [ count ] ); // 의존성배열에 대입된 count가 setCount가 될때 현재 useEffect (자동)실행
    // 3-3 : 1. 최초실행
    useEffect( () => { console.log('(3-3)최초실행') } , [ ] );
    // * 
    const [ count2 , setCount2 ] = useState( 0 );
    return (
        <>
        <h3> useEffect 예제1 : { count } { count2 } </h3>
